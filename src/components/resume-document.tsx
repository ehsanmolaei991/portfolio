...
/**
 * The résumé document itself, rendered by both `/resume` (base) and
 * `/resume/[variant]`.
 *
 * It used to read `?variant=` from `searchParams`, which forced the route to be
 * server-rendered on demand. Variants are a closed, known set, so they are now
 * static routes instead — that is what lets the entire site build as a static
 * export and deploy as plain files.
 */
...
export function ResumeDocument({ variant }: { variant?: string }) {
  const data = getResume(variant);
